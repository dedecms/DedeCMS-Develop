...
------------------------*/
    ...
    if ($dbtype == 'sqlite') {
        ...
        mysqli_select_db($dbname, $conn) or die("<script>alert('选择数据库失败，可能是你没权限，请预先创建一个数据库！');history.go(-1);</script>");

        //获得数据库版本信息
        $rs = mysqli_query("SELECT VERSION();", $conn);
        $row = mysqli_fetch_array($rs);
        $mysqlVersions = explode('.', trim($row[0]));
        $mysqlVersion = $mysqlVersions[0] . "." . $mysqlVersions[1];

        mysqli_query("SET NAMES '$dblang',character_set_client=binary,sql_mode='';", $conn);
    }
            ...
            if ($dbtype == 'sqlite') {
                ...
                if ($mysqlVersion < 4.1) {
                    ...
                    if (preg_match('#CREATE#i', $query)) {
                        $rs = mysqli_query(preg_replace("#TYPE=MyISAM#i", $sql4tmp, $query), $conn);
                    } else {
                        $rs = mysqli_query($query, $conn);
                    }
                }
        ...
        if (preg_match("#;$#", $line)) {
            if ($dbtype == 'sqlite') {
                ...
                if ($mysqlVersion < 4.1) {
                    ...
                    $rs = mysqli_query(str_replace('#~lang~#', $dblang, $query), $conn);
                }
    ...
    $dbtype == 'sqlite' ? $db->exec($cquery) : mysqli_query($cquery, $conn);
    ...
    $dbtype == 'sqlite' ? $db->exec($adminquery) : mysqli_query($adminquery, $conn);
...
------------------------*/
    ...
    if ($conn) {
        if (empty($dbname)) {
            ...
            $info = mysqli_select_db($dbname, $conn) ? "<font color='red'>数据库已经存在，系统将覆盖数据库</font>" : "<font color='green'>数据库不存在,系统将自动创建</font>";
            echo $info;
        }
    } else {
